 ...
 * Merk at klassene m� utvides for � l�se oppgaven.
 */
import java.util.*;
import java.time.LocalDate;

//Gjøres abtract siden ingen dyr (enkelt-individer) kan registreres som egen rase.
abstract class Art {
  ...
  /**
   * Konstrukt�ren Art: Ingen argumenter kan v�re null.
   * Ingen krav er satt til nye argumenter som kommer inn i subklassenes konstrukt�rer.
   */
  public Art(String norskNavn, String latNavn, String latFamilie) {
    if (norskNavn == null || latNavn == null || latFamilie == null) {
      throw new IllegalArgumentException("Ingen av argumentene til Art-konstrukt�ren kan v�re null");
    }
...
class DyrSomIndivider extends Art {
  ...
  public DyrSomIndivider(String norskNavn, String latNavn,
    String latFamilie, String antUnger, boolean farlig) {
  ...
  public String toString() {
    ...
    for (int i = 0; i < individer.size(); i++) {
        ...
        res += individer.get(i).getIndividnavn();
    }
    return res;
  }

  public boolean registrerIndivid(String navn, LocalDate fodselsdato) {
      for (Individ individ : individer) {
          if (individ.getIndividnavn().equals(navn)) {
              return false;
          }
      }
      individer.add(new Individ(navn, fodselsdato));
      return true;
  }

  public Individ finnIndivid(String navn) {
      for (Individ individ : individer) {
          if (individ.getIndividnavn().equals(navn)) {
              return individ;
          }
  ...
  public LocalDate finnFDatoIndivid(String navn) {
      for (Individ individ : individer) {
          if (individ.getIndividnavn().equals(navn)) {
              return individ.getFodselsdato();
          }
      }
      return null;
  }
...
class TestKlassetre {
  public static void main(String[] args) {
    DyrSomIndivider dyr = new DyrSomIndivider("Bj�rn", "XXX", "YYY", "1-2", true);
